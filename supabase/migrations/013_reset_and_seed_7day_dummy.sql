-- 013_reset_and_seed_7day_dummy.sql
-- Wipes all existing run/response/citation data and run summaries, then
-- regenerates a clean 7-day history with one run per day for each of the
-- 4 providers (claude, chatgpt, gemini, perplexity) — 28 runs total.

-- ============================================================
-- CLEAN SLATE
-- ============================================================
delete from runs; -- cascades to responses -> citations / entity_mentions
truncate table run_day_summaries;
...
begin
  ...
  foreach v_provider in array array['claude', 'chatgpt', 'gemini', 'perplexity'] loop
    for v_day in 0..6 loop
      v_run_date := (current_date - v_day) + case v_provider
        when 'claude'     then time '02:00:00'
      ...
      for v_prompt in select id, text, vertical from prompts where active = true loop
        ...
        if v_prompt.vertical = 'home_loans' then
          ...
          if v_provider = 'claude' then
            if random() > 0.15 then insert into citations (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_canstar,  'https://www.canstar.com.au/home-loans/',     'canstar.com.au',   1, true);  end if;
            if random() > 0.25 then insert into citations (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_finder,   'https://www.finder.com.au/home-loans',      'finder.com.au',    2, true);  end if;
            if random() > 0.3  then insert into citations (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_ratecity, 'https://www.ratecity.com.au/home-loans',    'ratecity.com.au',  3, false); end if;
            if random() > 0.4  then insert into citations (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_mozo,     'https://mozo.com.au/home-loans',            'mozo.com.au',      4, false); end if;
            if random() > 0.5  then insert into citations (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_commbank, 'https://www.commbank.com.au/home-loans.html','commbank.com.au',  5, false); end if;
            if random() > 0.6  then insert into citations (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_westpac,  'https://www.westpac.com.au/home-buying/',   'westpac.com.au',    6, false); end if;
            if random() > 0.65 then insert into citations (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_athena,   'https://www.athena.com.au/home-loans',      'athena.com.au',    7, false); end if;
            if random() > 0.7  then insert into citations (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_unloan,   'https://www.unloan.com.au/',                'unloan.com.au',    8, false); end if;
            if random() > 0.35 then
              insert into citations      (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_infochoice, 'https://www.infochoice.com.au/home-loans/', 'infochoice.com.au', 9, true);
              insert into entity_mentions(response_id, entity_id, linked, cited_separately)       values (v_response_id, id_infochoice, true, true);
            end if;
            if random() > 0.45 then
              insert into citations      (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_savings, 'https://www.savings.com.au/home-loans/', 'savings.com.au', 10, true);
              insert into entity_mentions(response_id, entity_id, linked, cited_separately)       values (v_response_id, id_savings, true, true);
            end if;
            if random() > 0.75 then
              insert into citations      (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_loans, 'https://www.loans.com.au/home-loans', 'loans.com.au', 11, true);
        ...
        -- ── CAR LOANS ───────────────────────────────────────────────────────
          ...
          if v_provider = 'claude' then
            ...
            if random() > 0.4  then insert into citations (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_ratecity, 'https://www.ratecity.com.au/car-loans', 'ratecity.com.au', 3, false); end if;
            if random() > 0.55 then insert into citations (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_commbank, 'https://www.commbank.com.au/car-loans.html','commbank.com.au',4,false); end if;
            if random() > 0.6  then insert into citations (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_nab,      'https://www.nab.com.au/personal/loans/car-loan','nab.com.au',5,false); end if;
            if random() > 0.6  then insert into citations (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_anz,      'https://www.anz.com.au/personal/personal-loans/car-loan/','anz.com.au',6,false); end if;
            if random() > 0.5  then
            ...
            if random() > 0.6  then
              insert into citations      (response_id, entity_id, url, domain, position, in_body) values (v_response_id, id_savings, 'https://www.savings.com.au/car-loans/', 'savings.com.au', 8, false);
